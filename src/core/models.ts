...
export interface CustomModel extends BaseModel {
    ...
    invoke?: (input: any, result: any) => unknown; // optional as to not be sent to backend
}
...
export interface RunTestProps {
    ...
    metrics_kwargs?: {[key: string]: string | number | boolean | undefined};
    tags?: string[];
    checks?: string[];
}
        ...
        if (mType === "custom") {
            ...
            */
            ...
            for (let i = 0; i < seed_data.length; i++) {
                ...
                const invoke = (this.models as CustomModel).invoke;
                if (invoke) {
                    // eslint-disable-next-line  @typescript-eslint/no-explicit-any
